...
function startProvider() {
  ...
  const server = createServer(async (req, res) => {
    ...
    requests += 1
    const id = `origin-compare-${requests}`
    const created = Math.floor(Date.now() / 1000)
    res.writeHead(200, { "content-type": "text/event-stream; charset=utf-8", "cache-control": "no-cache", connection: "keep-alive" })
    res.write(`data: ${JSON.stringify({ id, object: "chat.completion.chunk", created, model: "canary", choices: [{ index: 0, delta: { role: "assistant", content: "ORIGIN_COMPARE_DONE" }, finish_reason: null }] })}\n\n`)
...
function bridgeSource(targetHref, traceFile) {
  return [
    ...
    '  return event?.data?.sessionID ?? event?.properties?.sessionID ?? event?.sessionID ?? event?.data?.item?.sessionID',
    '}',
    'const diagnostic = {',
    '  id: target.id,',
    '  async setup(ctx) {',
...
async function main() {
  const temp = await mkdtemp(path.join(os.tmpdir(), "opencode-goals-v2-origin-compare-"))
  ...
  const traceFile = path.join(temp, "origin-compare.jsonl")
  ...
  await writeFile(path.join(pluginDir, "opencode-goals-v2-origin-compare.js"), bridgeSource(targetHref, traceFile))
  await writeFile(path.join(project, "README.md"), "# V2 origin comparison\n")
  await writeFile(path.join(project, "opencode.json"), `${JSON.stringify({
    $schema: "https://opencode.ai/config.json",
    model: "canary/canary",
    provider: { canary: { npm: "@ai-sdk/openai-compatible", name: "V2 origin compare", options: { baseURL: `http://127.0.0.1:${port}/v1`, apiKey: "canary" }, models: { canary: { name: "V2 origin compare", limit: { context: 100000, output: 4096 } } } } },
    command: { goal: { template: BRIDGE_TEMPLATE, description: "V2 origin compare", agent: "build", subtask: false } },
  }, null, 2)}\n`)

  const env = { ...process.env, HOME: home, USERPROFILE: home, XDG_CONFIG_HOME: config, XDG_DATA_HOME: data, XDG_STATE_HOME: state, OPENCODE_DB: path.join(data, "opencode", "origin-compare.db"), OPENCODE_LOG_LEVEL: "DEBUG", CI: "true" }
  await run("git", ["init", "-q"], { cwd: project, env })
  await run("git", ["config", "user.name", "V2 Origin Compare"], { cwd: project, env })
  ...
  try {
    ...
    const promptAdmission = await api("post", `/api/session/${encodeURIComponent(promptSessionID)}/prompt`, { prompt: { text: BRIDGE_TEXT }, delivery: "steer", resume: false })
    await sleep(1200)
    ...
    console.log(JSON.stringify({
      ...
      promptNormalized,
      identicalInbox,
      commandEvents: commandEvents.map(eventSummary),
      promptEvents: promptEvents.map(eventSummary),
