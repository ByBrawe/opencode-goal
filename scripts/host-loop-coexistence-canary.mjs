...
const GOAL_OBJECTIVE = "preserve Goal execution across Loop overlap rejection"
...
async function seedConfigDependencies(dir) {
  ...
  await writeFile(
    ...
    `${JSON.stringify({
      name: "opencode-goal-loop-coexistence-config",
      lockfileVersion: 3,
      requires: true,
...
function streamText(res, content, sequence) {
  const id = `chatcmpl-loop-coexistence-${sequence}`
...
function startProvider() {
  ...
  return {
    ...
    async listen() {
      ...
      const address = server.address()
      if (!address || typeof address === "string") throw new Error("failed to start Loop coexistence provider")
      return address.port
    },
    async close() {
...
async function main() {
  ...
  assert.ok(existsSync(loopPackageJson), "@bybrawe/opencode-loop must be installed before the coexistence canary")
  const loopPackage = JSON.parse(await readFile(loopPackageJson, "utf8"))

  const workspace = await mkdtemp(path.join(os.tmpdir(), "opencode-goal-loop-coexistence-"))
  ...
  await writeFile(path.join(workspace, "opencode.json"), `${JSON.stringify({
    ...
    provider: {
      canary: {
        ...
        name: "Deterministic Goal Loop Coexistence Canary",
        options: { baseURL: `http://127.0.0.1:${providerPort}/v1`, apiKey: "canary-key" },
        models: { canary: { name: "Deterministic Goal Loop Coexistence Canary", limit: { context: 100000, output: 4096 } } },
      },
    },
  }, null, 2)}\n`)
  ...
  try {
    ...
    const createdPayload = await api("/session", { method: "POST", body: JSON.stringify({ title: "Goal + Loop ownership canary" }) })
    ...
    await waitFor(() => provider.stats.goalRequests === 1, "the first Goal provider turn to be held open", diagnostics)
    await waitFor(async () => {
      const goal = await readGoal(workspace, sessionID)
      return goal?.status === "active" && goal?.objective === GOAL_OBJECTIVE && goal?.execution?.agent === "build"
    ...
    // OpenCode 1.18.x cancels the in-flight Goal turn to materialize the Loop
    // command bridge. A future host may honor noReply and leave the Goal stream
    // untouched instead. Both host shapes are safe as long as Goal ownership is
    // preserved and Loop never dispatches an autonomous prompt.
    ...
    if (provider.stats.goalClosed > 0) {
      await waitFor(() => provider.stats.commandBridgeRequests >= 1, "the compatibility Loop command bridge to execute", diagnostics, 20_000)
      await waitFor(() => provider.stats.goalRequests >= 2, "Goal to resume after the Loop command bridge", diagnostics, 20_000)
    }

    await waitFor(async () => {
      const goal = await readGoal(workspace, sessionID)
      return goal?.status === "active" && goal?.execution?.agent === "build"
    }, "Goal to remain active under its original executor", diagnostics, 20_000)

    const activeGoal = await readGoal(workspace, sessionID)
    assert.equal(provider.stats.loopRequests, 0, `Loop must not dispatch autonomously while dedicated Goal owns continuation\n${await diagnostics()}`)
    ...
    assert.equal(activeGoal?.execution?.agent, "build", "Loop command local agent must not replace Goal execution ownership")
