...
function todoPlanBlock(goal: GoalState): string {
  if (!goal.todoPlan) {
    return "Todo plan: not observed for this Goal revision."
  }
  const freshness = todoPlanIsCurrent(goal) ? "current" : "stale; rebuild it before relying on it"
  return `${todoPlanSourceLabel(goal)}: ${formatTodoPlan(goal)} (${freshness}).`
}
