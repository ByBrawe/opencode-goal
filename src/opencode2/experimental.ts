...
export const OpenCode2GoalsExperimental = {
  ...
  setup: async (ctx: OpenCode2ExperimentalContext) => {
    ...
    try {
      ...
      // Current V2 exposes the context hook. Keep a bounded fallback for older
      // experimental hosts while this adapter remains explicitly unstable.
    }
    if (!contextHookRegistered) await ctx.session.hook("request", handleRequest)
