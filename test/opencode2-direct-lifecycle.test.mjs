...
test("V2 direct Build lifecycle uses same host message id and keeps model-visible control read-only", async () => {
  ...
  try {
    await withPreview(async () => {
      ...
      assert.equal(host.prompts.length, 2, "status must not run the model")
      assert.equal(host.synthetics.length, 1)
      assert.match(host.synthetics[0].text, /Goal: ship docs/)
      assert.equal(host.synthetics[0].resume, false)
      ...
      assert.equal(host.prompts.length, 2)
      assert.match(host.synthetics.at(-1).text, /Status: paused/)
      ...
      assert.equal(host.prompts.length, 6)
      assert.equal(host.synthetics.at(-1).text, "Goal cleared.")
...
test("V2 direct Plan lifecycle persists paused and requires Build resume", async () => {
  ...
  try {
    await withPreview(async () => {
      ...
      assert.match(host.synthetics.at(-1).text, /saved but paused in plan mode/i)
      ...
      assert.match(host.synthetics.at(-1).text, /switch to Build and run \/goal resume/i)
...
test("V2 direct lifecycle fails closed on missing host context and pauses after continuation wake failure", async () => {
  ...
  try {
    await withPreview(async () => {
      ...
      assert.match(transport.synthetics.at(-1).text, /Goal paused because OpenCode 2 could not dispatch/i)
    })
  } finally {
    await rm(root, { recursive: true, force: true })
  }
