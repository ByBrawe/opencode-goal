...
const EXACT_ARGUMENTS = "host origin diagnostic"
...
async function run(command, args, { cwd, env, allowFailure = false, timeout = 90_000 } = {}) {
  return await new Promise((resolve, reject) => {
    ...
    child.stdout?.on("data", (chunk) => { stdout = (stdout + String(chunk)).slice(-160_000) })
    child.stderr?.on("data", (chunk) => { stderr = (stderr + String(chunk)).slice(-160_000) })
...
function startProvider() {
  ...
  const server = createServer(async (req, res) => {
    ...
    stats.requests.push({
      ...
      tools: Array.isArray(body?.tools) ? body.tools.map((item) => item?.function?.name ?? item?.name).filter(Boolean) : [],
    })
    const id = `surface-${stats.requests.length}`
    const created = Math.floor(Date.now() / 1000)
    res.writeHead(200, { "content-type": "text/event-stream; charset=utf-8", "cache-control": "no-cache", connection: "keep-alive" })
    res.write(`data: ${JSON.stringify({ id, object: "chat.completion.chunk", created, model: "canary", choices: [{ index: 0, delta: { role: "assistant", content: "SURFACE_DIAGNOSTIC_DONE" }, finish_reason: null }] })}\n\n`)
...
function bridgeSource(targetHref, traceFile) {
  return [
    ...
    'const keys = (value) => value && (typeof value === "object" || typeof value === "function") ? Reflect.ownKeys(value).map(String).slice(0, 80) : []',
    'const fn = (value) => typeof value === "function" ? { name: value.name, length: value.length, source: Function.prototype.toString.call(value).slice(0, 1400) } : { type: typeof value }',
    ...
    '    text: typeof part.text === "string" ? part.text.slice(0, 500) : undefined,',
    '  }',
    '}',
    'function messageShape(message) {',
    '  if (!message || typeof message !== "object") return { type: typeof message }',
    '  const info = message.info && typeof message.info === "object" ? message.info : undefined',
    '  const parts = Array.isArray(message.parts) ? message.parts : Array.isArray(message.content) ? message.content : Array.isArray(message.message?.parts) ? message.message.parts : []',
    '  return {',
    '    keys: keys(message),',
    '    infoKeys: keys(info),',
    '    role: message.role ?? info?.role,',
    '    id: message.id ?? info?.id,',
    '    sessionID: message.sessionID ?? info?.sessionID,',
    '    parentID: message.parentID ?? info?.parentID,',
    '    command: message.command ?? info?.command,',
    '    name: message.name ?? info?.name,',
    '    source: message.source ?? info?.source,',
    '    synthetic: message.synthetic ?? info?.synthetic,',
    '    mode: message.mode ?? info?.mode,',
    '    agent: message.agent ?? info?.agent,',
    '    parts: parts.map(partShape),',
    '  }',
    '}',
    ...
    '    agent: event?.agent ?? event?.agentID,',
    '    messages: Array.isArray(event?.messages) ? event.messages.map(messageShape) : { type: typeof event?.messages },',
    '    tools: event?.tools && typeof event.tools === "object" ? Object.keys(event.tools) : [],',
    '    systemType: Array.isArray(event?.system) ? `array:${event.system.length}` : typeof event?.system,',
    '  }',
    '}',
    'function domainSurface(domain) {',
    '  const result = { keys: keys(domain), methods: {} }',
    '  for (const key of keys(domain)) {',
    '    let value',
    '    try { value = domain[key] } catch (error) { result.methods[key] = { getterError: String(error) }; continue }',
    '    if (typeof value === "function") result.methods[key] = fn(value)',
    '  }',
    '  return result',
    '}',
    'const diagnostic = {',
    ...
    '      session: domainSurface(ctx.session),',
    '      tool: domainSurface(ctx.tool),',
    '      event: domainSurface(ctx.event),',
    '      command: domainSurface(ctx.command),',
    '    })',
    ...
    '                trace({ phase: "session-hook-enter", hook: name, event: eventShape(event) })',
    '                const result = Reflect.apply(callback, this, [event, ...callbackRest])',
    '                const after = () => trace({ phase: "session-hook-after", hook: name, event: eventShape(event) })',
...
async function main() {
  const temp = await mkdtemp(path.join(os.tmpdir(), "opencode-goals-v2-surface-"))
  ...
  const traceFile = path.join(temp, "surface-trace.jsonl")
  ...
  await writeFile(path.join(pluginDir, "opencode-goals-v2-surface.js"), bridgeSource(target, traceFile))
  await writeFile(path.join(project, "README.md"), "# V2 host surface diagnostic\n")
  await writeFile(path.join(project, "opencode.json"), `${JSON.stringify({
    $schema: "https://opencode.ai/config.json",
    model: "canary/canary",
    provider: { canary: { npm: "@ai-sdk/openai-compatible", name: "V2 surface", options: { baseURL: `http://127.0.0.1:${port}/v1`, apiKey: "canary" }, models: { canary: { name: "V2 surface", limit: { context: 100000, output: 4096 } } } } },
    command: { goal: { template: MANAGED_BRIDGE, description: "V2 managed bridge surface", agent: "build", subtask: false } },
  }, null, 2)}\n`)

  const env = { ...process.env, HOME: home, USERPROFILE: home, XDG_CONFIG_HOME: config, XDG_DATA_HOME: data, XDG_STATE_HOME: state, OPENCODE_DB: path.join(data, "opencode", "surface.db"), OPENCODE_LOG_LEVEL: "DEBUG", CI: "true" }
  await run("git", ["init", "-q"], { cwd: project, env })
  await run("git", ["config", "user.name", "V2 Surface"], { cwd: project, env })
  ...
  try {
    ...
    const created = await api("post", "/api/session", { title: "surface", agent: "build", model: { id: "canary", providerID: "canary" }, location: { directory: project } })
    ...
    throw new Error("diagnostic-only: inspect V2 host command-origin surfaces")
  } finally {
