...
function streamTool(res, sequence, name, args) {
  const id = `chatcmpl-v2-current-stable-${sequence}`
  ...
  send(res, {
    ...
    choices: [{
      ...
      delta: {
        ...
        tool_calls: [{
          index: 0,
          id: `call-v2-current-stable-${sequence}`,
          type: "function",
...
function streamText(res, sequence, text) {
  const id = `chatcmpl-v2-current-stable-text-${sequence}`
  const created = Math.floor(Date.now() / 1000)
  streamHeaders(res)
  send(res, {
    id,
...
function startProvider() {
  ...
  const server = createServer(async (req, res) => {
    ...
    streamText(res, sequence, "LIFECYCLE_SETTLED")
  })
...
async function main() {
  ...
  const workspace = await mkdtemp(path.join(os.tmpdir(), "opencode-goal-v2-current-stable-"))
  ...
  await writeFile(path.join(workspace, "README.md"), "# Current V2 stable lifecycle proof\n", "utf8")
  ...
  try {
    ...
    const created = await request("/api/session", {
      method: "POST",
      body: JSON.stringify({ title: "Current V2 stable lifecycle" }),
    })
