...
function validateProjectLocation(response, project) {
  if (response?._tag) {
...
async function waitForPluginActivation({ cwd, env, project, pluginPath, sentinelMarkerFile }) {
  let last = null
  for (let attempt = 1; attempt <= PLUGIN_READY_ATTEMPTS; attempt += 1) {
    const pluginResult = run("opencode2", ["api", "get", pluginPath], { cwd, env })
    const response = parseJSONOutput(pluginResult, "GET /api/plugin at project Location")
    validateProjectLocation(response, project)
    ...
    if (ids.includes(sentinelID) && ids.includes(pluginID) && sentinelMarker === "loaded\n") {
      return last
    }

    if (attempt < PLUGIN_READY_ATTEMPTS) {
      console.error(
        `OpenCode 2 project plugins not ready on attempt ${attempt}/${PLUGIN_READY_ATTEMPTS}; retrying same service in ${PLUGIN_READY_DELAY_MS}ms. ids=${JSON.stringify(ids)} marker=${sentinelMarker === "loaded\n"}`,
      )
      await new Promise((resolve) => setTimeout(resolve, PLUGIN_READY_DELAY_MS))
    }
  }
  return last
}
...
async function main() {
  ...
  try {
    ...
    const activation = await waitForPluginActivation({
      cwd: project,
      env,
      project,
      pluginPath,
    ...
    if (!activation) throw new Error("OpenCode 2 plugin activation readiness loop returned no result")

    const { attempt: activationAttempt, pluginResult, response, ids, sentinelMarker } = activation
    if (!ids.includes(sentinelID)) {
      throw new Error([
        `OpenCode 2 resolved the project Location but did not activate the minimal V2 { id, setup } plugin from .opencode/plugins/ after ${PLUGIN_READY_ATTEMPTS} same-service attempts.`,
    ...
    if (sentinelMarker !== "loaded\n") {
      throw new Error([
        `OpenCode 2 listed ${sentinelID}, but its setup() side effect did not run after ${PLUGIN_READY_ATTEMPTS} same-service attempts.`,
    ...
    if (!ids.includes(pluginID)) {
      throw new Error(`OpenCode 2 activated the V2 sentinel but not ${pluginID} after ${PLUGIN_READY_ATTEMPTS} same-service attempts; the Goals adapter module/setup is incompatible with this beta host. Active IDs: ${JSON.stringify(ids)}\nRaw response: ${String(pluginResult.stdout ?? "")}`)
    }
