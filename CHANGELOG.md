...
- Retry transient Windows `EPERM`, `EACCES`, and `EBUSY` access/share denials while reading canonical Goal process-lock owner bytes and inspecting the canonical hard-link metadata (`lstat`), using the same small bounded backoff window.
- Preserve fail-closed semantics across both paths: shell hashing/deduplication/revision ownership and completion evidence stay unchanged, while `ENOENT`, malformed lock metadata, symlink/junction checks, CAS/lease ownership, POSIX behavior, and persistent Windows access failures keep their existing authority.
- Add regressions proving three distinct timed-out shell turns still reach the normal stall pause; transient Windows owner-read and canonical-lock `lstat` denials recover to normal `lock_timeout`; persistent denials still escape unchanged.
