...
const OBJECTIVE = "prove real host compaction continuation"
...
function jsonText(res, content, sequence) {
  const id = `chatcmpl-compaction-canary-${sequence}`
...
function startProvider() {
  ...
  const server = createServer(async (req, res) => {
    ...
    const content = sequence === 2
      ...
      : sequence === 3
        ? "GOAL_CONTINUATION_AFTER_COMPACTION_OK"
        : sequence > 3
          ? `UNEXPECTED_EXTRA_REQUEST_${sequence}`
          : "INITIAL_GOAL_TURN_OK"

    if (body.stream) streamText(res, content, sequence)
    else jsonText(res, content, sequence)
  })
...
async function main() {
  ...
  const workspace = await mkdtemp(path.join(os.tmpdir(), "opencode-goal-real-compaction-canary-"))
  ...
  await writeFile(path.join(workspace, "opencode.json"), `${JSON.stringify({
    ...
    provider: {
      canary: {
        ...
        models: { canary: { name: "Deterministic Compaction Canary", limit: { context: 100000, output: 4096 } } },
      },
    },
  }, null, 2)}\n`)
  ...
  const diagnostics = () => `provider=${JSON.stringify({ chatRequests: provider.stats.chatRequests, requests: provider.stats.requests.map((item) => ({ sequence: item.sequence, userText: item.userText.slice(0, 500), hasCompactionContext: item.allText.includes(COMPACTION_CONTEXT_MARKER) })) }, null, 2)}\nserver log:\n${serverLog}`
  ...
  try {
    ...
    const createdPayload = await api("/session", { method: "POST", body: JSON.stringify({ title: "opencode-goal real compaction canary" }) })
    ...
    const activeBeforeCompaction = await waitForGoal(
      ...
      "active Goal state before manual compaction",
      diagnostics,
    )
    ...
    await waitFor(() => provider.stats.chatRequests >= 3, "compaction summary plus Goal-owned continuation", diagnostics)
    ...
    assert.equal(provider.stats.chatRequests, 3, `expected initial Goal turn, one compaction request, and one Goal continuation only\n${diagnostics()}`)
    ...
    assert.ok(compact.allText.includes(COMPACTION_CONTEXT_MARKER), `compaction request did not include persisted Goal context\n${diagnostics()}`)
    assert.ok(compact.allText.includes(OBJECTIVE), `compaction request lost the active Goal objective\n${diagnostics()}`)
    ...
    const transcriptText = messages.flatMap((message) => Array.isArray(message?.parts) ? message.parts.map(partText) : []).filter(Boolean).join("\n")
