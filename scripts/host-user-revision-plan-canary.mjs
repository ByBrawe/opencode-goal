...
const OLD_OBJECTIVE = "finish the original paused Goal project"
...
function startProvider() {
  ...
  const server = createServer(async (req, res) => {
    ...
    const allText = allMessageText(body)
    ...
    streamText(res, { id, created, content: "USER_REVISION_CANARY_UNEXPECTED_PROMPT" })
  })
...
async function main() {
  ...
  const workspace = await mkdtemp(path.join(os.tmpdir(), "opencode-goal-user-revision-plan-canary-"))
  ...
  const api = async (label, pathname, init = {}) => {
    ...
    try {
      ...
      throw new Error(`${label} failed: ${error?.stack || error}\n${diagnostics()}`)
    }
  }

  try {
    await waitForTcp(port, server, () => serverLog)
    const sessionsBefore = await api("session bootstrap", "/session", { method: "GET", signal: AbortSignal.timeout(45_000) })
    assert.ok(Array.isArray(sessionsBefore?.data ?? sessionsBefore), "GET /session bootstrap probe did not return a session array")

    const createdPayload = await api("session create", "/session", { method: "POST", body: JSON.stringify({ title: "opencode-goal paused user revision canary" }) })
    ...
    const goalCommand = api("Plan Goal create", `/session/${encodeURIComponent(sessionID)}/command`, {
      method: "POST",
      body: JSON.stringify({ agent: "plan", model: "canary/canary", command: "goal", arguments: `${OLD_OBJECTIVE} --max-turns 8` }),
    ...
    const paused = await waitForGoal(
      ...
      "Plan-created Goal revision 1 to persist paused",
      diagnostics,
    )
    lastState = paused
    await goalCommand
    ...
    const revised = await waitForGoal(
      ...
      (state) => state.sessionID === sessionID
        && state.revision === 2
        && state.status === "active"
        && state.execution?.agent === "build"
        && state.objective === `${OLD_OBJECTIVE}\n\nAdditional user instruction:\n${USER_EXTENSION}`
        && state.todoPlan === undefined,
      "foreground user extension to persist as active Goal revision 2",
      diagnostics,
      30_000,
    )
    lastState = revised
    ...
    assert.equal(provider.stats.sawRevisionTool, true, "custom user-revision tool was not exposed to the real OpenCode request")
    ...
    assert.equal(provider.stats.sawRevisionToolResult, true, "real host did not feed the revision tool result back into the foreground turn")
