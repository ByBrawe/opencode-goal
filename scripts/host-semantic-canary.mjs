...
const PROOF = "SEMANTIC_CANARY_PROOF"
const OBJECTIVE = "semantic canary contract"
...
async function seedConfigDependencies(dir) {
  ...
  await writeFile(
    ...
    `${JSON.stringify({
      name: "opencode-goal-semantic-canary-config",
      lockfileVersion: 3,
      requires: true,
...
function extractAuditRequest(text) {
  const token = text.match(/"auditToken"\s*:\s*"([^"]+)"/)?.[1]
  const requirementIDs = [...text.matchAll(/"id"\s*:\s*"([^"]+)"/g)].map((match) => match[1])
  if (!token || requirementIDs.length === 0) {
    throw new Error(`semantic canary could not parse verifier audit request:\n${text.slice(-6000)}`)
  }
  return { token, requirementIDs: [...new Set(requirementIDs)] }
}
...
function startProvider(readPath) {
  const stats = {
    chatRequests: 0,
    paths: [],
  ...
  const server = createServer(async (req, res) => {
    ...
    const id = `chatcmpl-semantic-${stats.chatRequests}`
    ...
    if (!body.stream) {
      ...
      res.end(JSON.stringify({ error: { message: "semantic canary expects streaming OpenCode requests" } }))
      return
    }
    ...
    if (tools.has("opencode_goal_verifier_result")) {
      ...
      if (tools.has("read") && !priorTools.includes("read")) {
        ...
        streamToolCall(res, {
          id,
          created,
          callID: `call-read-${stats.chatRequests}`,
          name: "read",
          args: { filePath: readPath },
        })
        return
      }
      ...
      if (!priorTools.includes("opencode_goal_verifier_result")) {
        ...
        streamToolCall(res, {
          id,
          created,
          callID: `call-verdict-${stats.chatRequests}`,
          name: "opencode_goal_verifier_result",
          args: {
            auditToken: request.token,
            results: request.requirementIDs.map((requirementID) => ({
              requirementID,
              verdict: "proven",
              reason: `README contains the authoritative ${PROOF} marker for the semantic canary objective.`,
      ...
      streamText(res, { id, created, content: "Verifier verdict submitted." })
      return
    }

    if (tools.has("opencode_goal_complete") && text.includes(OBJECTIVE)) {
      if (!priorTools.includes("opencode_goal_complete")) {
        stats.executorCompleteCalls += 1
        streamToolCall(res, {
          id,
          created,
          callID: `call-complete-${stats.chatRequests}`,
          name: "opencode_goal_complete",
          args: { summary: "Semantic canary completed with independently corroborated evidence." },
        })
        return
      }
      streamText(res, { id, created, content: "Semantic canary complete." })
      return
    }
  ...
  return {
    ...
    async listen() {
      ...
      const address = server.address()
      if (!address || typeof address === "string") throw new Error("failed to start deterministic semantic provider")
      return address.port
    },
    async close() {
...
async function main() {
  const workspace = await mkdtemp(path.join(os.tmpdir(), "opencode-goal-semantic-canary-"))
  ...
  const provider = startProvider(readme)
  ...
  await writeFile(readme, `# Semantic host canary\n\n${PROOF}\n`)
  await writeFile(path.join(workspace, "opencode.json"), `${JSON.stringify({
    ...
    provider: {
      canary: {
        ...
        name: "Deterministic Semantic Canary",
        options: { baseURL: `http://127.0.0.1:${providerPort}/v1`, apiKey: "canary-key" },
        models: { canary: { name: "Deterministic Semantic Canary", limit: { context: 100000, output: 4096 } } },
      },
    },
  }, null, 2)}\n`)
  ...
  try {
    ...
    const createdPayload = await api("/session", {
      method: "POST",
      body: JSON.stringify({ title: "opencode-goal semantic canary" }),
    })
    ...
    const command = api(`/session/${encodeURIComponent(sessionID)}/command`, {
      ...
      body: JSON.stringify({
        ...
        model: "canary/canary",
        command: "goal",
        arguments: `${OBJECTIVE} --contains "README.md::${PROOF}" --max-turns 8`,
      }),
    ...
    const completed = await waitForGoal(
      ...
      "semantic verifier-backed completion",
      diagnostics,
      30_000,
    ).catch(async (error) => {
    ...
    await command
    assert.ok(completed.requirements.length >= 2, "expected objective and declared file requirements")
    assert.ok(completed.requirements.every((item) => item.status === "proven"), `not all requirements were proven: ${JSON.stringify(completed.requirements)}`)
    assert.ok(completed.evidence.some((item) => item.trust === "host" && item.kind === "file" && item.passed === true), "missing current host file evidence")
    assert.ok(completed.evidence.some((item) => item.trust === "verifier" && item.kind === "file" && item.passed === true), "missing corroborated semantic verifier evidence")
    assert.equal(provider.stats.executorCompleteCalls, 1, "executor should attempt completion exactly once")
    assert.ok(provider.stats.verifierReadCalls >= 1, "verifier did not exercise the real OpenCode read tool")
    assert.equal(provider.stats.verifierResultCalls, 1, "verifier should submit exactly one verdict tool call")
    ...
    console.log(JSON.stringify({
      ...
      requirementStatuses: completed.requirements.map((item) => ({ verification: item.verification, status: item.status })),
      evidenceTrust: completed.evidence.map((item) => ({ trust: item.trust, kind: item.kind, passed: item.passed })),
    }, null, 2))
  } finally {
