...
async function seedConfigDependencies(dir) {
  ...
  await writeFile(
    ...
    `${JSON.stringify({
      name: "opencode-goal-steering-canary-config",
      lockfileVersion: 3,
      requires: true,
...
function startProvider() {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (text.includes(OLD_OBJECTIVE) && !text.includes(NEW_OBJECTIVE)) {
      stats.oldStarted += 1
      oldResponses.add(res)
      res.once("close", () => {
        if (oldResponses.delete(res)) stats.oldClosed += 1
      })
      streamHeaders(res)
      writeSse(res, {
        id,
        ...
        created,
        model: "canary",
        choices: [{ index: 0, delta: { role: "assistant", content: "WORKING_OLD" }, finish_reason: null }],
      })
      return
    }

    if (text.includes(NEW_OBJECTIVE)) {
      stats.newStarted += 1
      streamText(res, { id, created, content: "STEERED_NEW" })
      return
    }
  ...
  return {
    ...
    async listen() {
      ...
      const address = server.address()
      if (!address || typeof address === "string") throw new Error("failed to start steering provider")
      return address.port
    },
    async close() {
      for (const response of oldResponses) response.destroy()
...
async function main() {
  const workspace = await mkdtemp(path.join(os.tmpdir(), "opencode-goal-steering-canary-"))
  ...
  await writeFile(path.join(workspace, "opencode.json"), `${JSON.stringify({
    ...
    provider: {
      canary: {
        ...
        name: "Deterministic Steering Canary",
        options: { baseURL: `http://127.0.0.1:${providerPort}/v1`, apiKey: "canary-key" },
        models: { canary: { name: "Deterministic Steering Canary", limit: { context: 100000, output: 4096 } } },
      },
    },
  }, null, 2)}\n`)
  ...
  try {
    ...
    const createdPayload = await api("/session", { method: "POST", body: JSON.stringify({ title: "opencode-goal steering canary" }) })
    ...
    const oldCommand = api(`/session/${encodeURIComponent(sessionID)}/command`, {
      method: "POST",
    ...
    await waitFor(
      async () => {
        ...
        return provider.stats.oldStarted === 1 && lastState?.revision === 1 && lastState?.status === "active"
      },
      "old revision executor stream to become active",
      diagnostics,
    )
    ...
    await waitFor(
      async () => {
        lastState = await readGoal(workspace)
        return lastState?.revision === 2 && lastState?.objective === NEW_OBJECTIVE
      },
    ...
    await editCommand
    await oldCommand.catch(() => undefined)

    await waitFor(
      async () => {
        lastState = await readGoal(workspace)
        return lastState?.revision === 2 && lastState?.objective === NEW_OBJECTIVE && provider.stats.newStarted >= 2
      },
      "new revision to continue after its command turn",
      diagnostics,
    )
    ...
    assert.notEqual(lastState.status, "completed")
