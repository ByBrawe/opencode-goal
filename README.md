...
```json
{
  "plugin": ["@bybrawe/opencode-goal@1.3.4"]
}
```

OpenCode loads the package's dedicated `./server` entrypoint. The root package export remains the public JavaScript API and is intentionally separate from the server-plugin module.
...
If installation succeeded but `/goal` is missing, or if the model sees text beginning with `OpenCode Goals command bridge`, the command file loaded but the server plugin did not. Releases before `1.3.4` could hit this when OpenCode loaded the npm package root as a legacy plugin module.
...
3. Confirm the exact pin is `@bybrawe/opencode-goal@1.3.4` or newer.
...
The repository includes deterministic regression tests, adversarial evals, minimum/current OpenCode compatibility lanes, real-host lifecycle/semantic/Todo/steering canaries, restart recovery, cross-platform package smoke tests, server-entry regression coverage, and installer/update/uninstall tests.
