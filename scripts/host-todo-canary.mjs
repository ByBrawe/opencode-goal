...
const OBJECTIVE = "analyze this project, identify required gaps, and finish them"
const TODOS = [
  { content: "Inspect the current repository state", status: "in_progress", priority: "high" },
  { content: "Finish gaps required by the existing Goal", status: "pending", priority: "high" },
  { content: "Run verification for the completed work", status: "pending", priority: "medium" },
]
...
async function seedConfigDependencies(dir) {
  ...
  await writeFile(
    ...
    `${JSON.stringify({
      name: "opencode-goal-todo-canary-config",
      lockfileVersion: 3,
      requires: true,
...
function startProvider() {
  ...
  const server = createServer(async (req, res) => {
    ...
    const id = `chatcmpl-todo-${stats.chatRequests}`
    ...
    if (!body.stream) {
      ...
      res.end(JSON.stringify({ error: { message: "Todo canary expects streaming OpenCode requests" } }))
      return
    }

    if (text.includes(OBJECTIVE)) {
      assert.ok(tools.has("todowrite"), `real Build agent did not expose native todowrite; tools=${[...tools].join(",")}`)
      stats.sawTodoGuidance ||= text.includes("<todo_orchestration>") && text.includes("Todo completion itself still proves nothing")
      if (!priorTools.includes("todowrite")) {
        stats.todoCalls += 1
        streamToolCall(res, {
          id,
          created,
          callID: `call-todo-${stats.chatRequests}`,
          name: "todowrite",
          args: { todos: TODOS },
        })
        return
      }
      streamText(res, { id, created, content: "Native Todo plan captured; Goal remains unfinished." })
      return
    }

    streamText(res, { id, created, content: "TODO_CANARY_OK" })
  })
  ...
  return {
    ...
    async listen() {
      ...
      const address = server.address()
      if (!address || typeof address === "string") throw new Error("failed to start deterministic Todo provider")
      return address.port
    },
    async close() {
...
async function main() {
  ...
  const workspace = await mkdtemp(path.join(os.tmpdir(), "opencode-goal-todo-canary-"))
  ...
  await writeFile(path.join(workspace, "README.md"), "# Native Todo host canary\n")
  await writeFile(path.join(workspace, "opencode.json"), `${JSON.stringify({
    ...
    provider: {
      canary: {
        ...
        name: "Deterministic Todo Canary",
        options: { baseURL: `http://127.0.0.1:${providerPort}/v1`, apiKey: "canary-key" },
        models: { canary: { name: "Deterministic Todo Canary", limit: { context: 100000, output: 4096 } } },
      },
    },
  }, null, 2)}\n`)
  ...
  try {
    ...
    const command = api(`/session/${encodeURIComponent(sessionID)}/command`, {
      ...
      body: JSON.stringify({
        ...
        model: "canary/canary",
        command: "goal",
        arguments: `${OBJECTIVE} --max-turns 8`,
      }),
    ...
    const planned = await waitForGoal(
      ...
      (state) => state.sessionID === sessionID
        ...
        && state.status === "active"
        && state.todoPlan?.goalRevision === state.revision
        && state.todoPlan?.total === TODOS.length,
      "native Todo telemetry bound to the active Goal revision",
      diagnostics,
      30_000,
    )

    assert.equal(provider.stats.todoCalls, 1, "deterministic executor should invoke native todowrite exactly once before pause")
    assert.equal(provider.stats.sawTodoGuidance, true, "executor did not receive Goal Todo orchestration guidance")
    assert.equal(planned.todoPlan.pending, 2)
    assert.equal(planned.todoPlan.inProgress, 1)
    assert.equal(planned.todoPlan.completed, 0)
    assert.equal(planned.progressRevision, 0, "native Todo planning must not count as host-observed Goal progress")
    assert.equal(planned.observedProgressRevision, 0, "native Todo planning must not move the observed progress watermark")
    assert.deepEqual(planned.evidence, [], "native Todo planning must not create Goal completion evidence")
    assert.ok(planned.requirements.every((item) => item.status === "pending"), "native Todo planning must not prove Goal requirements")
    ...
    const paused = await waitForGoal(
      workspace,
      (state) => state.sessionID === sessionID && state.status === "paused",
      "Goal pause after native Todo telemetry",
      diagnostics,
      15_000,
    )
    await command

    assert.equal(paused.todoPlan?.digest, planned.todoPlan.digest, "pausing must preserve advisory Todo telemetry")
    assert.equal(paused.progressRevision, 0)
    assert.deepEqual(paused.evidence, [])
    if (commandError && !String(commandError).toLowerCase().includes("abort")) throw commandError
    ...
    console.log(JSON.stringify({
      ...
      sawTodoGuidance: provider.stats.sawTodoGuidance,
      todoPlan: paused.todoPlan,
      progressRevision: paused.progressRevision,
      evidenceCount: paused.evidence.length,
