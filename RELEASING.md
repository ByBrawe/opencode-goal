...
5. when publication is still needed, requires the predecessor release (`1.3.32`) to exist and remain authoritative as npm `latest` with the expected installer bin before allowing `1.3.33` to publish.
...
- the exact `1.3.33` package version is visible in the npm registry;
- npm `latest` resolves to `1.3.33`;
- `bin.opencode-goal` resolves to the expected `bin/opencode-goal.js` path;
- a clean consumer can run `npm exec --yes --package=@bybrawe/opencode-goal@1.3.33 -- opencode-goal --version` and receives `1.3.33`.
