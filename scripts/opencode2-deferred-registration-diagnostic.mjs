...
function startProvider() {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (request === 1 && observation.hasControl) {
      send({
        ...
        choices: [{
          ...
          delta: {
            role: "assistant",
            content: null,
            tool_calls: [{ index: 0, id: "call-deferred-control", type: "function", function: { name: CONTROL_TOOL, arguments: "" } }],
          },
          finish_reason: null,
        }],
      })
      ...
      send({ id, object: "chat.completion.chunk", created, model: "canary", choices: [{ index: 0, delta: { role: "assistant", content: "DEFERRED_REGISTRATION_DONE" }, finish_reason: null }] })
...
function bridgeSource(targetHref, traceFile) {
  return [
    ...
    '  } catch (error) { trace({ phase: "deferred.register.error", error: String(error) }); throw error }',
    '}',
    'const diagnostic = {',
    '  id: target.id,',
    '  async setup(ctx) {',
    '    const stream = typeof ctx.event?.subscribe === "function" ? ctx.event.subscribe() : null',
    '    if (stream) {',
    '      void (async () => {',
    '        try {',
    ...
    '            trace({ phase: "plugin.added", data: event?.data })',
    '            if (raw.includes("opencode.config.policy")) await registerDeferred(ctx)',
    '          }',
...
async function main() {
  const temp = await mkdtemp(path.join(os.tmpdir(), "opencode-goals-v2-deferred-"))
  ...
  const traceFile = path.join(temp, "deferred-trace.jsonl")
  ...
  await writeFile(path.join(pluginDir, "opencode-goals-v2-deferred.js"), bridgeSource(targetHref, traceFile))
  await writeFile(path.join(project, "README.md"), "# V2 deferred registration diagnostic\n")
  await writeFile(path.join(project, "opencode.json"), `${JSON.stringify({
    $schema: "https://opencode.ai/config.json",
    model: "canary/canary",
    provider: { canary: { npm: "@ai-sdk/openai-compatible", name: "V2 deferred", options: { baseURL: `http://127.0.0.1:${port}/v1`, apiKey: "canary" }, models: { canary: { name: "V2 deferred", limit: { context: 100000, output: 4096 } } } } },
    command: { goal: { template: DECLARED_TEMPLATE, description: "V2 deferred goal", agent: "build", subtask: false } },
  }, null, 2)}\n`)

  const env = { ...process.env, HOME: home, USERPROFILE: home, XDG_CONFIG_HOME: config, XDG_DATA_HOME: data, XDG_STATE_HOME: state, OPENCODE_DB: path.join(data, "opencode", "deferred.db"), OPENCODE_LOG_LEVEL: "DEBUG", CI: "true" }
  await run("git", ["init", "-q"], { cwd: project, env })
  await run("git", ["config", "user.name", "V2 Deferred"], { cwd: project, env })
  ...
  try {
    ...
    await waitFor(async () => (await readTrace(traceFile)).includes('"phase":"deferred.register.done"') ? true : null, "deferred transform registration")
    ...
    }, "deferred random Goal wrapper to become authoritative")

    const created = await api("post", "/api/session", { title: "deferred", agent: "build", model: { id: "canary", providerID: "canary" }, location: { directory: project } })
    ...
    throw new Error(`diagnostic-only: firstControl=${Boolean(first?.hasControl)} firstWrapper=${Boolean(first?.sawWrapper)} goalPersisted=${Boolean(goal)} secondControl=${Boolean(second?.hasControl)}`)
  } finally {
